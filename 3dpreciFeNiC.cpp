...
double y_fraction_c_bcc;
double y_fraction_x_bcc; 
double y_fraction_x_fcc;
double y_fraction_c_fcc;
double dtempsFeCCr,tempsFeCCr;
int a1,a2,def;//lattice 


void calc3dequilibreFeNiC();
bool cinetiqueLE(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &prevVectorcin, bool &flage);
bool calc3cinetiqueFeNiC_growth(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &prevVectorcin,bool &flage);
bool calc3cinetiqueFeNiC_complet(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &prevVectorcin,bool &flage);
bool cinetiqueLE_ch(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &prevVectorcin, bool &flage);
bool calc3cinetiqueFeNiC_growth_ch(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &prevVectorcin,bool &flage);
bool cinetiqueLE_qh(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &Ycinetique_para_ch, bool &flage_ch);
bool calc3cinetiqueFeNiC_BalanceC(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &prevVectorcin,bool &flage);
bool calc3cinetiqueFeNiC_para_or_LE(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &prevVectorcin,bool &flage_ch);
bool calc3cinetiqueFeNiC_para_or_LE_T(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &prevVectorcin,bool &flage_ch);

void print_vect(std::vector <double> &Y);
void C_YtoX( std::vector <double> &Y,std::vector <double> &X);
void C_YatoXa( std::vector <double> &Y,std::vector <double> &X);
void C_XtoU( std::vector <double> &X,std::vector <double> &U);
void C_XatoUa( std::vector <double> &X,std::vector <double> &U);
void C_X0toU0(std::vector <double> &X0, std::vector <double> &U0);
double Sursaturation_c(std::vector <double> &U,std::vector <double> &U0);
double Sursaturation_ni(std::vector <double> &U,std::vector <double> &U0);
...
std::vector <double> BILAN_MASSE_PLANE_U(std::vector <double> &U,double dtempsFeCCr);
...
void C_YtoX( std::vector <double> &Y,std::vector <double> &X)
{
X[1] = Y[1]/(1.+a2*Y[2]); //Ni_bcc
X[2] = a2*Y[2]/(1.+a2*Y[2]);//C_bcc
X[0] = 1-X[1]-X[2];//Fe_bcc
X[4] = Y[5]/(1.+Y[6]);//Ni_fcc
X[5] = Y[6]/(1.+Y[6]);//C_fcc	
X[3] = 1-X[4]-X[5];//Fe
X[6] = Y[8];//Velocity	
}
...
void C_YatoXa( std::vector <double> &Y,std::vector <double> &X)
{
X[0] = 3.*Y[0]/(1.+3.*Y[0]);//C_bcc
X[1] = Y[2]/(1.+Y[2]);//C_fcc	
X[2] = Y[1]/(1.+3.*Y[0]);//Ni_bcc
X[3] = Y[3]/(1.+Y[2]);//Ni_fcc	
}

void C_XatoUa( std::vector <double> &X,std::vector <double> &U)
{
U[0] = X[3]/(1.-X[1]);//Ni_fcc
U[1] = X[2]/(1.-X[0]);//Ni_bcc
U[2] = X[1]/(1.-X[1]);//C_fcc
U[3] = X[0]/(1.-X[0]);//C_bcc
}
...
void C_XtoU( std::vector <double> &X,std::vector <double> &U)
{
U[0] = X[4]/(1.-X[5]);//Ni_fcc
U[1] = X[1]/(1.-X[2]);//Ni_bcc
U[2] = X[5]/(1.-X[5]);//C_fcc
U[3] = X[2]/(1.-X[2]);//C_bcc
}
...
{
U0[0]=X0[0]/(1.-X0[0]);//C0
U0[1]=X0[2]/(1.-X0[0]);//NI0	
}

double Sursaturation_c(std::vector <double> &U,std::vector <double> &U0)
{
return (U[2]-U0[0])/(U[2]-U[3]);
}

double Sursaturation_ni(std::vector <double> &U,std::vector <double> &U0)
{
return (U[0]-U0[1])/(U[0]-U[1]);
}
...
CONCENTRATION_NOMINAL[0] = 0.3624e-2;// carbon -C (fraction mole)
CONCENTRATION_NOMINAL[2] = 1.92e-2;// nickel -Ni (fraction mole)
...
CONCENTRATION_NOMINAL[0] = 0.52e-2;// carbon -C (fraction mole)
CONCENTRATION_NOMINAL[2] = 2.4e-2;// nickel -Ni (fraction mole)
CONCENTRATION_NOMINAL[1] = 1.-CONCENTRATION_NOMINAL[0]-CONCENTRATION_NOMINAL[2];// iron -FE

std::vector <double> Ma(3);
Ma[0]=1.2011e1;
Ma[1]=5.5847e1;
Ma[2]=5.8690e1;
	
std::vector <double> test(3);
test = CONCENTRATION_MOLEtoCONCENTRATION_MASS(CONCENTRATION_NOMINAL,Ma);
//test = CONCENTRATION_MASStoCONCENTRATION_MOLE (CONCENTRATION_NOMINAL,Ma);
	
printf("%e %e %e\n",test[0],test[1],test[2]);	
exit(0);
	
	
printf ( "Begin calcul FE-Ni-C system : \n" );
...
if((T>=400.) && (T<=1100.))
cinetiqueLE(CONCENTRATION_NOMINAL,Y,prevVectorcin,flage);
}
...
//Principle
printf("Kinetic Fe-C-Ni with local equilibrium at interface \n\n\n");	
...
calc3cinetiqueFeNiC_growth(CONCENTRATION_NOMINAL,Ycinetique,prevVectorcin_para,flage_para);
//calc3cinetiqueFeNiC_BalanceC(CONCENTRATION_NOMINAL,Ycinetique,prevVectorcin_para,flage_para);
//calc3cinetiqueFeNiC_para_or_LE(CONCENTRATION_NOMINAL,Y,prevVectorcin_para,flage_para);
}
...
//calc3cinetiqueFeNiC_complet(CONCENTRATION_NOMINAL,Ycinetique_comp,prevVectorcin,flage_ch);
//calc3cinetiqueFeNiC_para_or_LE_T(CONCENTRATION_NOMINAL,Ycinetique_comp,prevVectorcin,flage_ch);
...
bool flage_ch = false;	
calc3cinetiqueFeNiC_complet(CONCENTRATION_NOMINAL,Ycinetique_veloc,prevVectorcin,flage_ch);	
}
	

bool calc3cinetiqueFeNiC_complet(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &prevVectorcin,bool &flage_ch)
{	
...
double surNi = 	Sursaturation_ni(Ucinetique,Unom);
double surC  = Sursaturation_c(Ucinetique,Unom);	
...
//v						//r			//Delta_ni
U[7]=Ycinetique_para[4];U[8]=Rzin;U[9]=2.*Rzin*(1.-surNi)/surNi;
//Delta_c							//time
U[10]=2*Rzin*(1.-surC)/surC;U[11]=tempsFeCCr;	
//U0ni			//U0c			//Dni/Dni
...
cinetiqueLE_ch(CONCENTRATION_NOMINAL,prevVectorcin,Ycinetique_para_ch,flage_ch);	
//cinetiqueLE_qh(CONCENTRATION_NOMINAL,prevVectorcin,Ycinetique_para_ch,flage_ch);	
...
bool cinetiqueLE_ch(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &Ycinetique_para_ch, bool &flage_ch)
{
newt nt;
nt=newt();
int i=1;
...
calc3cinetiqueFeNiC_growth_ch(CONCENTRATION_NOMINAL,Ycinetique,Ycinetique_para_ch,flage_ch);	
}	

}

bool calc3cinetiqueFeNiC_growth_ch(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y,std::vector<double> &Ycinetique_para_ch,bool &flage)
{
printf("V\n");	
newt nt;
nt=newt();
...
std::vector <double> BILAN_MASSE_PLANE_U(std::vector <double> &U,double dtempsFeCCr)
{
	...
	//U[6]=Uc0
//printf("here\n");	
double Ac, Ax, deltac, Ln, deltax;
vector <double>	Cu(2);
Ln = 20.;	
deltac = (U[2] - U[6])/(U[2] - U[3]); 	
deltax = (U[0] - U[5])/(U[0] - U[1]);
/*printf("deltac = %e deltax = %e \n",deltac,deltax);
printf("dc = %e dni = %e \n",dc,dcr);
printf("L-Rzin =%e v = %e \n",Ln-Rzin,U[4]);
printf("dtemps=%e \n",dtempsFeCCr);	
Ac = exp(-dtempsFeCCr*(dc/deltac-U[4])/(Ln-Rzin));
Ax = exp(-dtempsFeCCr*(dcr/deltax-U[4])/(Ln-Rzin));
printf("Ac = %e Ax = %e \n",Ac,Ax); 	
Cu[0] = U[0] - Ax;//X	
Cu[1] = U[2] - Ac;//C
*/
////%%%%%%%
Ac = (U[2] - U[6])*dtempsFeCCr*(dc/deltac-U[4])/(Ln-Rzin);	
Ax = (U[0] - U[5])*dtempsFeCCr*(dcr/deltax-U[4])/(Ln-Rzin);	
printf("Ac = %e Ax = %e \n",Ac,Ax);	
//Cu[0] = U[0] - Ax;//X	
//Cu[1] = U[2] - Ac;//C	
Cu[0] = U[5] - Ax;//X	
...
bool cinetiqueLE_qh(std::vector <double> &CONCENTRATION_NOMINAL,std::vector <double> &Y1,std::vector<double> &Ycinetique_para_ch, bool &flage_ch)
{
printf("Equilibrium\n");	
//The difinition dimension of alloy
int dim=3;
...
cinetiqueLE_ch(CONCENTRATION_NOMINAL,Yinput,Ycinetique_para_ch,flage_ch);
}
else {
	return 0;
}
